...
- 搜索结果排序：小红书按当前页可见点赞数从高到低取前 N 条；抖音仅在能稳定读取点赞数时排序，否则保留页面原顺序。
...
1. 双击项目目录里的 `start_web.bat`。启动器会让服务在后台运行；以后再次双击会直接打开网页，不会重复启动。
...
- 双击 `start_food_demo.bat` 打开美食情报页；如果 8787 上已有旧服务，启动器会使用 8788，不会结束现有进程。
- 通用采集页仍位于 `/`，美食情报页位于 `/food`。
- 美食衍生结果保存到 `D:\ObsidianVault\采集\美食情报\YYYY-MM-DD\<run_id>`，真实采集、任务书验收夹具、规则结果和人工审核状态分开标记。
- “热门”仅表示同平台、同关键词、同次可见候选池中的相对热门；不代表平台全站热榜。

公开演示版：<https://socialreader-food-agent.vercel.app>

- 公开页读取已归档、脱敏后的演示快照，不需要本机服务、登录态或外部 API。
- 实时采集仍在本地执行；公开页不会把快照演示包装成实时抓取结果。
- 演示源码和数据构建脚本位于 `public-demo/`。

离线验证：

```powershell
D:\Python312\python.exe -m unittest discover -s D:\SocialReader\tests\food_v1 -p "test_stage*.py" -v
D:\Python312\python.exe D:\SocialReader\domain_pipeline.py analyze "D:\ObsidianVault\采集\内容流水线\三文鱼波奇饭" --limit 3 --include-acceptance-fixtures
```
